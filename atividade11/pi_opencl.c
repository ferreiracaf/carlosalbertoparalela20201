...
int main (int argc, char *argv[]) {
	...
	if (WINDOWS) fopen_s(&programHandle, "leibdnizPi.cl", "r");             // Alterei para fopen_s
	if (LINUX) programHandle = fopen("leibdnizPi.cl", "r");
	...
	ckKernel = clCreateKernel(cpProgram, "leibdnizPi", &ciErr);
	...
	ciErr |= clSetKernelArg(ckKernel, 1, sizeof(cl_int), (void*)&iNumElements);
