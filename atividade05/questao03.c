...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        int trabalhoLocal = vetorTamanho / size;

        for (int i = 1; i < size; i++){
            MPI_Send(vetorA + trabalhoLocal, trabalhoLocal, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
            MPI_Send(vetorB + trabalhoLocal, trabalhoLocal, MPI_FLOAT, i, 1, MPI_COMM_WORLD);
        }
        ...
        for (int i = 1; i < size; i++){
            MPI_Status status;
            MPI_Recv(vetorC + (trabalhoLocal * i), trabalhoLocal, MPI_FLOAT, i, 0, MPI_COMM_WORLD, &status);
        }
